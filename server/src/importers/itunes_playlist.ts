...
function toValue(type: 'boolean', value: string | XMLDictMap | XMLArray): boolean;
function toValue(type: 'integer', value: string | XMLDictMap | XMLArray): number;
function toValue(type: string, value: string | XMLDictMap | XMLArray): XMLValue {
...
async function decode(xml: Buffer): Promise<Playlist> {
  return new Promise((resolve, reject) => {
    ...
    let keyStack: string[] = [];
    let tagStack: string[] = [];
    let valueStack: string[] = [];
    let arrayStack: XMLArray[] = [];
    ...
    parser.on('endElement', function (name) {
      ...
      if (last !== name) throw new Error('Something wrong with tag stack order');
      previousTag = name;
      ...
      if (parent === 'dict') {
        ...
        value && dictStack[dictStack.length - 1]?.set(key, toValue(name, value));
        return;
      }
      if (parent === 'array') {
        let value;
